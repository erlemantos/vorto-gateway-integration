...
import org.eclipse.vorto.cloudservice.Activator;
import org.eclipse.vorto.cloudservice.Logger;
...
public class HonoMqttClient {
	...
	public HonoMqttClient(String mqttHostUrl, String clientId, String user, String password) {
		this.mqttHostUrl = Objects.requireNonNull(mqttHostUrl);
		this.clientId = Objects.requireNonNull(clientId);
		
		try {
			logger.info(String.format("Creating Hub Client for [%s] with Client Id [%s] and user [%s]", mqttHostUrl, clientId, user));
		...
		try {
		  CertificateFactory certFactory = CertificateFactory.getInstance("X.509");
			...
			InputStream certificateInputstream = new BufferedInputStream(
			    Activator.context.getBundle().getResource("iothub.crt").openStream());
